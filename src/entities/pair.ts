...
export class Pair {
  ...
  public static getAddress(tokenA: Token, tokenB: Token, factoryAddressOverride?: string, initCodeHashManualOverride?: string): string {
    const factoryAddress = factoryAddressOverride || (FACTORY_ADDRESS_MAP[tokenA.chainId] ?? FACTORY_ADDRESS)
    return computePairAddress({ factoryAddress, tokenA, tokenB, initCodeHashManualOverride })
  }

  public constructor(currencyAmountA: CurrencyAmount<Token>, tokenAmountB: CurrencyAmount<Token>, factoryAddressOverride?: string, initCodeHashManualOverride?: string) {
    const tokenAmounts = currencyAmountA.currency.sortsBefore(tokenAmountB.currency) // does safety checks
      ? [currencyAmountA, tokenAmountB]
      : [tokenAmountB, currencyAmountA]
    this.liquidityToken = new Token(
      tokenAmounts[0].currency.chainId,
      Pair.getAddress(tokenAmounts[0].currency, tokenAmounts[1].currency, factoryAddressOverride, initCodeHashManualOverride),
      18,
      'UNI-V2',
      'Uniswap V2'
    )
